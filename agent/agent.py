...
# 장애 시나리오 메트릭
nginx_5xx_total = Gauge(
    "infra_nginx_5xx_total", "Nginx 5xx error count during simulation"
)
db_active_connections = Gauge(
    "infra_db_active_connections", "Active DB connections during simulation"
)
db_max_connections = Gauge("infra_db_max_connections", "DB max_connections setting")
container_oom_killed = Gauge(
    "infra_container_oom_killed", "Number of OOM-killed containers"
)
db_deadlock_count = Gauge(
    "infra_db_deadlock_count", "DB deadlock count during simulation"
)
zombie_process_count = Gauge(
    "infra_zombie_process_count", "Zombie process count in container"
)
...
LOAD_TEST_STATUS_FILE = os.getenv(
    "LOAD_TEST_STATUS_FILE", "/tmp/auto-healing-load-test/status.json"
)
...
def update_metrics():
    while True:
        ...
        cpu_usage.set(
            int(forced_cpu)
            if forced_cpu is not None and forced_cpu != ""
